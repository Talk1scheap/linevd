...
sp = svd.get_dir(svd.processed_dir() / f"evasion_{config['embtype']}" / run_id)
logger_dir = svd.get_dir(svd.processed_dir() / f"logger"/ f"evasion_{config['embtype']}_{config['gtype']}_{config['multitask']}")
logger_csv = CSVLogger(logger_dir, name="evasion")
...
def train_linevd(
    ...
    # Load data
    data = lvd.BigVulDataset_EvasionTest(
        batch_size=config["batch_size"],
        sample=samplesz,
        methodlevel=False,
        nsampling=True,
