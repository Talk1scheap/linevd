...
class GGNNBatchGraph(BatchGraph):
    def get_network_inputs(self, cuda=False, device=None):
        ...
        if cuda:
            ...
            return self.graph, features.cuda(device=device), node_types.cuda(device=device), edge_types.cuda(device=device)
        else:
            return self.graph, features, node_types, edge_types
        pass